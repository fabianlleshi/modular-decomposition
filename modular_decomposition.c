...
/* ===================== Factorizing permutation (linear via LexBFS) ===================== */

/* --------- LexBFS in O(n+m) (Rose–Tarjan–Lueker) ---------
...
static void lbfs_order(const Graph *G, int *order) {
    ...
    for (int idx = n - 1; idx >= 0; idx--) {
        ...
        for (int t = G->off[s]; t < G->off[s + 1]; t++) {
            ...
            mark[u] = 1;
            LBBlock *blk = owner[u];
            if (blk && !blk->touched) {     // ← guard null owner
                blk->touched = 1;
                touched[tcount++] = blk;
            }
        }
        ...
        for (int i = 0; i < tcount; i++) {
            ...
            /* If we created a predecessor block but it ended up empty,
            unlink it from the list *before* freeing. */
            if (Bp && Bp->head == -1) {
            ...
            /* If the original block is now empty, unlink it and free later. */
            if (blk->head == -1) {
...
/* ===================== Assembly (incremental, global certification) ===================== */
